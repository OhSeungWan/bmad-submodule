...
| 환경 | 설계 문서 위치 | playwright/e2e/ 조회 위치 | 비고 |
|------|-------------|-------------------|------|
| 메인 디렉토리 | `{implementation_artifacts}/tests/` | 메인 `playwright/e2e/` | design 단계 기본 |
| 워크트리 | 메인의 `{implementation_artifacts}/tests/` | 워크트리 `playwright/e2e/` | implement/verify 단계 기본 |
...
2. 없으면 defaults 사용 (`["src", "app"]`, `playwright/e2e`)
...
# 기존 mock 데이터
find playwright/e2e/ -path "*/mocks/*.json"
```
...
```
playwright/
├── support/                          # 공통 자산 (전 feature 공유)
│   ├── merged-fixtures.ts            # 공통 fixture (viewport, 인증 등)
...
> **POM 재사용**: 특정 POM이 여러 feature에서 공유될 경우 `support/pages/`로 승격한다.

## 4. Page Object 설계

### {PageA}Page

- **파일**: `playwright/e2e/pages/{page-a}.page.ts`
- **담당 URL**: `/path/to/page`
- **Locator 전략**:
...
기존 `merged-fixtures.ts`에 추가할 POM 및 Mock Fixture 등록:
...
# Spec별 개별 실행
npx playwright test playwright/e2e/{feature-a}.spec.ts --project=chromium
npx playwright test playwright/e2e/{feature-b}.spec.ts --project=chromium
```
...
# 구현 후 테스트 실행
npx playwright test playwright/e2e/ --project=chromium
```
```
...
- playwright/e2e/ 디렉토리에 어떤 파일도 쓰지 않는다
